<script>
  ...
  function updateHex(hexStr, isInitialCall = false) {
    ...
    if (isValidHex) {
      ...
      if (isInitialCall) {
        warnMessage = `That hex code doesn't look right`; // TODO: i18n
...
<div class="main-container">
  ...
  <div class="color-info-box">
    ...
    <div class="hex-text-block">
      <input
        class="text"
        on:change={ev => updateHex(ev.target.value)}
        value={hexValue}
        maxlength="7" />
    </div>
...
{#if warnMessage}
  <div class="warning">{warnMessage}</div>
{/if}
